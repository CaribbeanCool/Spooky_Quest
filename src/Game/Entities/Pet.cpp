...
Pet::Pet(string id, string entityName, int ox, int oy) : Entity(ox, oy, 50, 64, 420, 220, 97, 125, 0, 0) {
    this->id = id;
    ...
    sprite.load("images/entities/Pets.png");
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 4; ++j) {
            temp.cropFrom(sprite,  x * i,  y * j, w - 5, h - 8);
            if (j == 0)
